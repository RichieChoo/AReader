...
class AppComponent extends PureComponent {
  ...
  render() {
    ...
    const { page, pageSize, gender, major, minor, minArr,type } = this.state;
        ...
        <div className={styles.listWrapper}>
            ...
            <Radio.Group value={major ? major : minors[0] && minors[0].major} buttonStyle="solid">
              {
                minors.map(v => {
                  return (
                    <Radio.Button key={v.major} onClick={(e) => this.handleChangeRadio(e, v)} value={v.major}>{v.major}</Radio.Button>
                  );
                })
              }
            </Radio.Group>
                ...
                <Radio.Group value={minor ? minor : mins[0]} buttonStyle="solid">
                  {
                    mins.map(v => {
                      return (
                        <Radio.Button key={v} onClick={this.handleChangeRadioMin} value={v}>{v}</Radio.Button>
                      );
                    })
                  }
                </Radio.Group>
...

          {
            books.books ? <List
              ...
              renderItem={item => {
                const cover = item.cover.includes("/agent/") ? decodeURIComponent(item.cover.replace(/\/agent\//, "")) : decodeURIComponent(item.cover);
                return (
                  <List.Item className={styles.listItem}>
                    <Row gutter={16}>

                      <Col span={6} className={styles.left}>
                        <img alt={item.title} src={cover} ref={this.imgRef} onError={this.handleResolveImg}/>
                      </Col>
                      <Col span={18} className={styles.right}>
                        <p>{item.title}</p>
                        <p>{item.author}&nbsp;&nbsp;|&nbsp;&nbsp;{item.majorCate}</p>
                        <p>{item.shortIntro.length > 88 ? item.shortIntro.substring(0, 88)+"..." : item.shortIntro}</p>
                        <p><span>{item.latelyFollower}</span>&nbsp;&nbsp;人气&nbsp;&nbsp;|&nbsp;&nbsp;<span>{item.retentionRatio}&nbsp;%&nbsp;&nbsp;</span>留存率</p>
                      </Col>
                    </Row>
                  </List.Item>
                );
